...
class Distinguish(nn.Module):
    def __init__(self):
        ...
        self.disca = DisCA()
        self.dissa = DisSA()
        self.fc = nn.Sequential(nn.ReLU(inplace=True), nn.Dropout(), nn.Linear(512, 2))
        self.avgpool = models.resnet18(weights="IMAGENET1K_V1").avgpool
...
class CA(Module):
    def __init__(self):
        ...
        self.alpha = Parameter(torch.zeros(1))
        self.softmax = Softmax(dim=-1)
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1), nn.LeakyReLU(), nn.BatchNorm2d(256))
        self.conv2 = nn.Sequential(nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, padding=1), nn.LeakyReLU(), nn.BatchNorm2d(256))
        self.conv3 = nn.Sequential(nn.Conv2d(in_channels=256, out_channels=512, kernel_size=3, padding=1), nn.LeakyReLU(), nn.BatchNorm2d(512))
...
class Model(nn.Module):
    def __init__(self, num_classes):
        ...
        self.getfeature1 = nn.Sequential(
            models.resnet18(weights="IMAGENET1K_V1").conv1,
            models.resnet18(weights="IMAGENET1K_V1").bn1,
            models.resnet18(weights="IMAGENET1K_V1").relu,
            models.resnet18(weights="IMAGENET1K_V1").maxpool,
            models.resnet18(weights="IMAGENET1K_V1").layer1,
            models.resnet18(weights="IMAGENET1K_V1").layer2,
            models.resnet18(weights="IMAGENET1K_V1").layer3,
            models.resnet18(weights="IMAGENET1K_V1").layer4,
        )
